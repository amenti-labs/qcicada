...
class QCicada:
    ...
    def drain_input(self) -> int:
        ...
        This is mainly useful immediately after entering continuous mode or
        after a mode switch when the first subsequent read should come from
        freshly generated bytes.
        """
        try:
            return self._transport.drain_input()
        except Exception as exc:
            raise QCicadaError(f'Failed draining input buffer: {exc}') from exc

    def start_continuous_fresh(self) -> int:
        """Start continuous mode and discard any already-buffered bytes.
