...
impl QCicada {
    ...
    pub fn get_statistics(&mut self) -> Result<DeviceStatistics, QCicadaError> {
        let data = self
            .command(CMD_GET_STATISTICS, None)?
            .ok_or(QCicadaError::Protocol(
                "No response to GET_STATISTICS".into(),
            ))?;
    ...
    /// Drain any bytes already queued in the serial input buffer.
    ///
    /// This is primarily useful immediately after entering continuous mode or
    /// after a mode switch, when callers want the first subsequent read to
    /// come from freshly generated bytes instead of previously buffered data.
    ///
    ...
    /// Start continuous mode and discard any bytes already buffered.
    ///
    /// This is the recommended entry point when callers care about the first
    /// read being as fresh as possible.
    ///
    /// Returns the number of discarded bytes.
    pub fn start_continuous_fresh(&mut self) -> Result<usize, QCicadaError> {
    ...
    pub fn get_verified_pub_key(&mut self, ca_pub_key: &[u8]) -> Result<Vec<u8>, QCicadaError> {
        let info = self.get_info()?;
        let dev_pub_key = self.get_dev_pub_key()?;
        let certificate = self.get_dev_certificate()?;

        let (hw_major, hw_minor) =
            crate::protocol::parse_hw_version(&info.hw_info).ok_or_else(|| {
        ...
        let serial_int = crate::protocol::parse_serial_int(&info.serial).ok_or_else(|| {
            QCicadaError::Protocol(format!("Cannot parse serial number from '{}'", info.serial))
        })?;
    ...
    pub fn signed_read_verified(
        ...
        let valid =
            crate::crypto::verify_signature(device_pub_key, &result.data, &result.signature)
                .map_err(|e| {
...
impl io::Read for QCicada {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let data = self.random(n).map_err(io::Error::other)?;
