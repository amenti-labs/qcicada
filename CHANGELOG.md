...
- **Fresh-start continuous mode helpers** — Added `start_continuous_fresh()`
  and `drain_input()` in both the Rust and Python SDKs. These helpers are for
  callers that want the first continuous read after mode entry to come from
  freshly generated bytes rather than already-buffered serial input.
